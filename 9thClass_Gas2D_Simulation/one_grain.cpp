...
int main(){
    ...
    Grain[N+0].init(Lx/2, Ly+RWall, 0, 0, MWall, RWall); // Up Wall
    Grain[N+1].init(Lx/2, -RWall, 0, 0, MWall, RWall); // Down Wall
    Grain[N+2].init(Lx+RWall, Ly/2, 0, 0, MWall, RWall); // Right Wall
    Grain[N+3].init(-RWall, Ly/2, 0, 0, MWall, RWall); //  Left Wall

    for(ix = 0; ix < Nx; ix++)
        for(iy = 0; iy < Ny; iy++){
            theta = 2 * M_PI * rand64.r();
            Grain[Nx*iy+ix].init((ix+1)*dx, (iy+1)*dy, 0, 0, m0, R0);
        }
